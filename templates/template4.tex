...
% This template provides  academic document styling with consistent design
...
\usepackage{tocloft}
\usepackage{setspace}
\usepackage{graphicx}  % For image support in cover page
...
\newcommand{\templateFourCover}[7]{%
  % #1: Document Title
  % #2: Document Subtitle  
  % #3: Year
  % #4: Submitted to (recipient)
  % #5: Prepared by (author)
  % #6: University logo path (optional - use empty {} if no logo)
  % #7: College logo path (optional - use empty {} if no logo)
  \begin{titlepage}
  \newgeometry{margin=0.7in,footskip=40pt}
  \thispagestyle{empty}
  ...
  % Professional header with logos (optional - from raw3)
  % If logo paths are provided and files exist, they will be displayed
  \ifx\relax#6\relax
    % No university logo path provided
    \vspace{2cm}
  \else
    \IfFileExists{#6}{
      ...
      \hfill
      \ifx\relax#7\relax
        % No college logo path provided
        \begin{minipage}[t]{0.45\textwidth}
            \raggedleft
            % Placeholder for second logo
            \textcolor{templateDark}{\large\textbf{Institution Logo}}
        \end{minipage}
      \else
        \IfFileExists{#7}{
          \begin{minipage}[t]{0.45\textwidth}
      ...
      % University logo file doesn't exist - skip logo section
      \vspace{2cm}
    }
  \fi
  ...
  {\Large\textit{\textcolor{templateAccent}{Advanced Technical Document}}}
  \end{center}
